...
```mermaid
graph TD
    subgraph Conceptual["三层认知视角"]
        direction TB
        L1["实现层 (Implementation): LLM + Context + Tools"]
        L2["直觉层 (Intuition): 大脑 (Brain) + 眼睛 (Eyes) + 手脚 (Hands & Feet)"]
        L3["学术层 (RL Concept): 策略 (Policy) + 观测空间 (Observation) + 动作空间 (Action)"]
        L1 --- L2
        L2 --- L3
    end
```
...
graph TD
    subgraph Early["早期 Agent 孤岛"]
        A1[Deep Research: 检索空间]
        A2[Coding Agent: 终端/文件空间]
        A3[Computer Use: 屏幕像素空间]
    end

    subgraph Manus["Manus: 三界合一"]
        M[统一大动作空间: 虚拟浏览器 + 文件系统 + 代码执行沙箱]
    end

    subgraph OpenClaw["OpenClaw: 进驻用户真实数字生活"]
        OC[多渠道网关 IM/Slack + 本地网关 + 云端驱动 Google Drive/Notion]
    end
...
```mermaid
timeline
    title 模型的吞噬与 Harness 的迁移前沿
    过去 : Few-shot 与提示技巧 : 被指令微调与推理模型内置
    近处 : 工具调用协议与格式修复 : 被原生 Tool Calling 与结构化输出内置
    当前 : 浏览器操作与中断交互 : 正在被多模态与全双工模型内置
    未来 : Harness 继续后退 : 迁移至企业级专属安全、动态审计与复杂业务兜底
```
